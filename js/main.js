...
function debugAjax(){
	...
	//use anonymous function for data conversion to JSON
    	.then(function(response){
            return response.json();
        }) 
        .then(debugCallback) //send retrieved data to the callback function
		
};
	
//define callback function
function debugCallback(response){ //converted data sent as response variable to be used in function
//insert the reponse into the HTML with description of GeoJSON data:
	document.querySelector("#mydiv").insertAdjacentHTML('beforeend', '<br>GeoJSON data:<br> ' + JSON.stringify(response))
};

//add event listener to execute debugAjax 
